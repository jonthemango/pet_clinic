...
public class TableDataGateway {
    ...
    public void insertPet(Pet pet) {
        db.execute(String.format("INSERT INTO pets (name, birth_date, type_id, owner_id) VALUES ('%s','%s',%d,%d)",
    ...
    public void insertVetSpecialty(Vet vet, Specialty specialty) {
        db.execute(String.format("INSERT INTO vet_specialties (vet_id, specialty_id) VALUES (%d, %d)", vet.getId(),
                specialty.getId()));
    }

    public void insertVet(Vet vet) {
        db.execute(String.format("INSERT INTO vets (first_name, last_name) VALUES ('%s', '%s')", vet.getFirstName(),
                vet.getLastName()));
    ...
    public void insertVisit(Visit visit) {
        db.execute(String.format("INSERT INTO visits (pet_id, visit_date, description) VALUES (%d, '%s', '%s')",
                visit.getPetId(), visit.getDate().toString(), visit.getDescription()));
    }

    public Object getById(Integer id, String tableName) {
        ResultSet resultSet = db.select(String.format("SELECT * FROM %s WHERE id = %d", tableName, id));
        try {
